      ...
      <b-tabs card>
        <b-tab title="Website" active>
          <b-form-row>
            <b-col>
              <b-form-group id="category"
                ...
                <b-form-select id="categoryInput" 
                              v-model="newScholarship.category_ref" 
                              :options="categories" 
                              class="" />
              </b-form-group>
            </b-col>
            <b-col v-if="newScholarship.category_ref !== 3">
            ...
            <b-col v-if="newScholarship.category_ref === 1">
              <b-form-group id="handler"
                          label="Manage Online"
                          label-for="handlerInput">
                <b-button id="handlerInput"
                          :pressed.sync="applicationHandler" 
                          :variant="applicationHandler ? 'success' : 'danger'">
                  {{ applicationHandler ? "True" : "False" }}
                </b-button>
              </b-form-group>
            </b-col>
            <b-col>
...
export default {
  ...
  data() {
    return {
      ...
      categories: [
        { text: 'Scholarship Category', value: 0, disabled: true },
  ...
  methods: {
    onSubmit(e) {
      this.axios.put('https://boc22finaid.fau.edu/scholarship/api/scholarship/', this.newScholarship)
      .then((res) => {
        console.log(res);
      }).catch((err) => {
        console.log(err);
