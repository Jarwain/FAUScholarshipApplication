...
try{
	...
	$user = "schapp";
	$pass = "schapp";
	$dbname = "scholarship_applications_1617";
        ...
        if(isset($_GET['submit']) && $_GET['submit'] == 1){
            if($_SESSION['znumber'] == NULL){
                if($_SESSION['scholarship'] == NULL){
                    ...
                    $applicationSELECT = $link->prepare("SELECT A.`response`,A.`summary`,A.`submitted`,S.`znumber`,S.`code`,S.`accepted`,S.`ranking`,S.`note`,B.`question`,B.`type`,C.`name`,C.`description`,C.`counter`,C.`limit`,F.`id`,F.`name` as `fname` 
                        FROM `answer` A
  ...
  <body>
    ...
    <div class="container" id="main">
      <div class="page-header">
        ...
        <h1>Financial Aid <small>Scholarship Application for 2016-2017</small></h1>
      </div>
     <?php if(!$isActive): ?>
  		<h2><?= $closedText ?></h2>
  		<?php 
