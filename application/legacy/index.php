...
// If the zNumber is not set, clear the session
$validLocation = ["verify","selection"];
if(empty($_SESSION['znumber']) || isset($_SESSION['location']) && !in_array($_SESSION['location'],$validLocation)){
  session_unset();
}
$_SESSION['location'] = "index"; /* Session variable to prevent users from bypassing index, and to track the user */

// $_SESSION['term'] = "FALL 2014"; /* Session variable changeable according to term & year */ // NOT USED

// if user was directed here for a specific application (index.php?scholarship=SFA###), set it for them
if(isset($_GET['scholarship'])){
  $_SESSION['scholarship'] = $_GET['scholarship'];
} 
// Otherwise, set it to null
elseif(!isset($_SESSION['scholarship'])){
...
<?php include "footer.php" ?>
    <script type="text/javascript">  
      <?php
        /* 
          When the Form is submitted, it goes to verify.php to be validated. If verify.php finds an issue, it is redirected back here, and valid triggers the validation 
        */
